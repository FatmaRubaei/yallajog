...
function AddSegmentDialog({ types, onSuccess }: { types: any[]; onSuccess: () => void }) {
  ...
  const [form, setForm] = useState({
    ...
    typeId: "",
    description: "",
    measurement: "duration" as "duration" | "distance",
    defaultDurationMinutes: "",
    defaultDistanceKm: "",
    intensityTarget: "none" as "none" | "pace",
    paceMin: "",
    paceSec: "",
  });
  ...
  function reset() {
    setForm({ name: "", typeId: "", description: "", measurement: "duration", defaultDurationMinutes: "", defaultDistanceKm: "", intensityTarget: "none", paceMin: "", paceSec: "" });
  }
    ...
    try {
      await mutation.mutateAsync({
        data: {
          ...
          defaultDurationMinutes: form.measurement === "duration" && form.defaultDurationMinutes ? Number(form.defaultDurationMinutes) : undefined,
          defaultDistanceKm: form.measurement === "distance" && form.defaultDistanceKm ? Number(form.defaultDistanceKm) : undefined,
          defaultPace: form.intensityTarget === "pace" ? formatPace(form.paceMin, form.paceSec) : null,
        },
      });
