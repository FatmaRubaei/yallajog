...
import { useGetTraineeBalance, useListTraineeTransactions, useGetTrainee } from "@workspace/api-client-react";
  ...
  return (
    <View style={[styles.txRow, { borderBottomColor: colors.border }]}>
      ...
      <View style={styles.txInfo}>
        ...
        {item.notes && (
          <Text style={[styles.txNotes, { color: colors.mutedForeground }]}>{item.notes}</Text>
        )}
...
export default function BillingScreen() {
  ...
  const { data: trainee } = useGetTrainee(
    traineeId ?? 0,
    { query: { enabled: !!traineeId } }
  );
    ...
    >
      ...
      <View style={[styles.summaryCard, { backgroundColor: isOverdue ? colors.destructive : colors.success, }]}>
        ...
        <Text style={styles.summaryStatus}>
          {isOverdue ? "Payment required" : "All paid up"}
        </Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  summaryLabel: { fontSize: 14, fontFamily: "Inter_500Medium", color: "rgba(255,255,255,0.8)", marginBottom: 8 },
  summaryAmount: { fontSize: 40, fontFamily: "Inter_700Bold", color: "#fff", marginBottom: 4 },
  summaryStatus: { fontSize: 13, fontFamily: "Inter_400Regular", color: "rgba(255,255,255,0.85)" },
  detailsRow: {
    marginHorizontal: 20,
    borderRadius: 12,
    borderWidth: 1,
  ...
  detailValue: { fontSize: 17, fontFamily: "Inter_700Bold" },
  detailLabel: { fontSize: 12, fontFamily: "Inter_400Regular", marginTop: 4 },
  ...
  txInfo: { flex: 1 },
  txMonth: { fontSize: 14, fontFamily: "Inter_500Medium" },
  txNotes: { fontSize: 12, fontFamily: "Inter_400Regular", marginTop: 2 },
